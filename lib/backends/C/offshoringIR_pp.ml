...
let rec cnv_typ ?(specs=[]) : typ -> C.ctype = function
  ...
  | TNum C32 -> to_ctype ~specs (C.Tnamed "float complex")
  | TNum C64 -> to_ctype ~specs (C.Tnamed "double complex")
  | TArray1 tp 
  | TRef    tp -> to_ctype ~specs (C.Tptr (cnv_typ tp))
  | TLenArray1 (len,tp) -> 
      to_ctype ~specs (C.Tarray (len,cnv_typ tp))
...
let rec cnv_init : exp -> C.init_expression = function
  | Array [] -> Init_none               (* in C23, {} is allowed and
                                           means the same as default init *)
  | Array els -> 
      Init_compound C.(List.map (fun e -> (Indes_none, cnv_init e)) els)
  | e -> Init_single (cnv_exp e)

let cnv_attributes : attribute list -> C.spec list = List.filter_map @@ function
  | A_static -> Some C.S_static
  | _        -> None

let cnv_def : binding -> C.let_or_statement = function
  | (None,e) -> C.STMT (cnv_exp_as_stmt e)
  | (Some {id;ty;mut=Cnst;attrs},(Array _ as e)) ->
      C.(LET (((id :> string),cnv_typ ~specs:(cnv_attributes attrs) ty), 
              cnv_init e))
  | (Some {id;ty;mut=Mut},Array _ ) ->
      (* should convert to mutable var of pointer type *)
      failwith "mutable var of composite type are not yet implemented"
  | (Some {id;ty;mut=Cnst;attrs},e) ->
      C.(LET (((id :> string),
               cnv_typ ~specs:(S_const::cnv_attributes attrs) ty), cnv_init e))
  | (Some {id;ty;mut=Mut;attrs},e) ->
      C.(LET (((id :> string),cnv_typ ~specs:(cnv_attributes attrs) ty),
              cnv_init e))

(* Convert a block *)
(* Generally speaking, even if the function returns non-void,
  lacking of the return is not an error. 
  It is an undefined behavior if the caller of the function uses the value.
  For example,
   int foo(void) {}
 compiles without errors or warnings even given the flag -W
 (there is a warning if given -Wall)
 void foo(void) { return 1;}
 gives a warning -- still, it is not an error.
 *)
